//https://github.com/SokuDev/SokuMods/blob/master/modules/WindowResizer/WindowResizer.cpp
#include <windows.h>
#include <string>
#include <Shlwapi.h>
...
#define IS_KEY_DOWN(lParam) ((HIWORD(lParam) & KF_REPEAT) == 0 && (HIWORD(lParam) & KF_UP) == 0)
#define IS_KEY_UP(lParam) ((HIWORD(lParam) & KF_UP) != 0)
...
void toggleFullscreen() {
	...
	{
		if (tryToHideCursor) ShowCursor(0);
...
LRESULT CALLBACK keyboardHook(int nCode, WPARAM wParam, LPARAM lParam)
{
	if (nCode == HC_ACTION)
	{
		...
		if (wParam >= '1' && wParam <= '6' 
			&& IS_KEY_UP(lParam) //strangely, alt+2 has no keydown if DISCL_NOWINKEY disabled
...
void WINAPI hotkeyThread()
{
	while (true) {
		if (fullscreen 
		&& ((GetAsyncKeyState(VK_TAB) && GetAsyncKeyState(VK_MENU)) 
