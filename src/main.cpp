...
LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam)//ÆÁ±Î Win ¼üµ¥»÷
{
	if (nCode == HC_ACTION 
		&& GetForegroundWindow() == SokuLib::window
	) {
		KBDLLHOOKSTRUCT* p = (KBDLLHOOKSTRUCT*)lParam;
		// ÆÁ±Î Win ¼ü
		if (p->vkCode == VK_LWIN || p->vkCode == VK_RWIN) {
			if (wParam == WM_KEYUP || wParam == WM_SYSKEYUP) {
				printf("WH_KEYBOARD_LL: Win key up blocked\n");
				return 1; // ·µ»Ø·ÇÁã£¬³ÔµôÏûÏ¢
			}
		}
		// ÆÁ±Î Alt+Space£¨Õë¶Ôpowertoys£¿£©
		
	}
	return CallNextHookEx(g_hHook, nCode, wParam, lParam);
}
inline void HookInstall()
{
	if (g_hHook) {
		return;
	}
	g_hHook = SetWindowsHookEx(WH_KEYBOARD_LL, LowLevelKeyboardProc, g_hDll, 0);//really wierd workaround with AltDrag.exe
}

inline void HookRemove()
{
	if (g_hHook) {
...
//typedef int(__thiscall SokuLib::Logo::* pfLogoIV)();
...
//	_endthread();
//}
unsigned char* const ADDR_DINPUT_SETCOOPERATIVE_SPEC = (unsigned char*)0x40d8b4;
